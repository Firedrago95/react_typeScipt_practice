...
*   **화살표 함수 (`() => {}`)**: Java의 람다식(`() -> {}`)과 매우 유사한 JavaScript/TypeScript의 함수 선언 문법입니다.
*   **React 함수형 컴포넌트**: HTML처럼 생긴 UI 코드(JSX/TSX)를 리턴하는 함수입니다. 함수 이름의 첫 글자는 반드시 대문자여야 합니다.
*   **JSX / TSX**: 코드 내부에서 HTML 태그를 직접 작성하여 UI를 표현할 수 있게 해주는 확장 문법입니다.
*   **배열 렌더링 (`map()`)**: Java의 `Stream.map()`과 같이 배열 요소를 돌며 다른 형태로 가공하는 함수입니다. React에서는 데이터 배열을 HTML 태그 배열로 변환하는 데 씁니다.
*   **`key` 속성의 필요성**: 루프를 돌며 요소를 그릴 때, React가 가상 DOM의 변경 사항을 추적할 수 있도록 최상단 태그에 유니크한 `key` 값을 지정해 주어야 합니다.
